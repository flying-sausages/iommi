...
class Column(Part):
    ...
    def on_refine_done(self):
        self.header = HeaderColumnConfig(**self.header).refine_done()
...
class Table(Part, Tag):
    ...
    def on_refine_done(self):
        ...
        if self.model:
            ...
            self.query = self.get_meta().query_class(
                filters=filters, _name='query', model=self.model, **self.query_args
            )
    ...
    def on_bind(self) -> None:
        ...
        declared_query_filters = declared_members(self.query)['filters'] if self.query is not None else {}
        declared_bulk_fields = declared_members(self.bulk)['fields'] if self.bulk is not None else {}
    ...
    def _bind_query(self):
        ...
        self.query = self.refine(filters=declared_filters)
    ...
    def _bind_bulk_form(self):
        ...
        self.bulk = self.bulk.refine(fields=declared_fields)
